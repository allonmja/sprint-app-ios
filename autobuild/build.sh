...
if [ "$Target" == "SPRINT" ]; then
    ...
    if [ "$Config" == "AutobuildRelease" ]; then
        /usr/bin/xcrun -sdk iphoneos PackageApplication -v $WORKSPACE/build.sym/${Config}-iphoneos/${Target}.app -o $WORKSPACE/build.sym/${Config}-iphoneos/${Target}.ipa --sign "iPhone Distribution: Compuware Corporation" --embed ~/Downloads/Enterprise.mobileprovision mv $WORKSPACE/build.sym/${Config}-iphoneos/${Target}.ipa $WORKSPACE/${Target}.ipa
    fi
fi
